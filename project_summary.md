...
*   **Two-Stage Full-Screen Launch Sequence:** On app startup, Stage 1 displays a dedicated **Company Logo Splash Screen** (`_CompanySplashScreen`) featuring the **MINDFUL MATRIX INTERACTIVE STUDIOS** shield emblem, Orbitron typography, and a `PRESENTS` tag (~2s). It then cross-fades into Stage 2 (`_FullScreenSplashScreen`) displaying the **FOCUS SPARK** game title, **Cosmic Spark Emblem**, and a 3-second animated progress loading bar (`INITIALIZING MATRIX... 100%`) before revealing the Home Screen.
