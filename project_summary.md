...
*   **Glassmorphic Level Completed Victory Modal:** After successfully completing a level sequence, a glassmorphic celebration dialog (`_showLevelCompletedModal()`) pops up featuring a glowing trophy badge (`Icons.emoji_events_rounded`), Orbitron completion title (`LEVEL X COMPLETED!`), milestone rank unlocks at L5, L10, L15, L20, L25, and L30, performance HUD (Next Target Level, Streak 🔥, High Score 🏆), and a primary **`CONTINUE TO LEVEL X ▶`** button alongside a **`VIEW ROADMAP 🗺️`** button. Fixed mobile `SharedPreferences` cache discrepancy by implementing self-healing initialization (`_loadSettings()`), capping `displayBestScore` to completed levels, and calculating `effectiveBest` against cleared level (`_level - 1`).
...
*   **Full-Screen Ad Lifecycle Pause & Resume Engine:** Integrated `onAdOpened` and `onAdClosed` lifecycle listeners in `AdService` and `_FocusSparkScreenState`. When a Rewarded or Interstitial ad displays, ambient background music automatically pauses (`AudioService.instance.stopAmbientMusic()`), input timers stop (`_cancelInputTimer()`), and game state freezes (`_gameState = GameState.paused`). Fixed restart/new session ad timing so new game initialization (`launchGame()`) is strictly deferred to the `onDismissed` callback of the Interstitial ad, preventing background pattern playback while watching ads.
...
*   **Separate Audio Header Controls (`🎵 Music` / `🔊 SFX`):** Dedicated header buttons for **Music (`🎵`)** (toggles ambient background pad `_isMusicMuted`) and **SFX (`🔊`)** (toggles tile chimes & victory SFX `_isSfxMuted`), with full `SharedPreferences` persistence.
